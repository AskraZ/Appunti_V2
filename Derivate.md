...
$a < x_{0} + h < b\iff a-x_{0}<h<b-x_0$      $h\neq 0$
...
Una funzione f è **strettamente crescente** in un punto $x_0$​ se e solo se esiste un intorno $I(x_0​,s)$ di raggio $s>0$ tale che, per ogni $x$ appartenente all'intorno (con $x\neq x_0​)$, il rapporto incrementale è strettamente positivo:
###### DIMOSTRAZIONE 
$r(x)=\frac{f(x)-f(x_0)}{x-x_0}$ 
se $r(x)>0 \ \ \forall x\in I(x_0,s)$ 

in $]x_0-s,x_{0}[ \begin{cases}r(x)>0 \\  x-x_0<0\end{cases}\implies f(x)-f(x_0)<0$ 

in $]x_{0},x_{0}+s[ \begin{cases}r(x)>0 \\  x-x_0>0\end{cases}\implies f(x)-f(x_0)>0$

viceversa se $f$ è crescente in $x_0$ 
in $]x_{0}-s, x_{0}[ \begin{cases}f(x)-f(x_{0})<0 \\  x- x_{0}<0\end{cases}\implies r(x)>0$

in $]x_{0}, x_{0}+s[ \begin{cases}f(x)-f(x_{0})>0 \\  x-x_0>0\end{cases}\implies r(x)>0$
...
$f(x)=|x|$  $(x_0=0)$
$r(x)=\frac{|x|-0}{x-0}=\begin{cases}1 & x>0 \\  -1 &  x<0\end{cases}\implies \not\exists \lim_{x\to 0}r(x)$

##### CRITERIO DI MONOTONIA LOCALE 
se $f'(x) > 0\implies f$ è crescente in $x_{0}$.
infatti $f'(x_{0})>0 \implies r(x)>0$ in un intorno di $x_{0}$.
##### CRITERIO DI MONOTONIA
se $f'(x)>0 \ \ \ \forall x \in(a,b) \implies f$ è strettamente crescente in $(a,b)$ (infatti lo è in ogni punto).
...
punto di flesso -> una funzione ha tangente verticale se $f''(x)$ cambia segno

punto cuspidale -> in una cuspide, la funzione è continua in $x_0$, ma le derivate da destra e da sinistra tendono entrambe a infinito con segni opposti.
...
- $f$ cresce strettamente in $(a,b)$ se $x<y\implies f(x)<f(y)$
- f cresce strettamente in $x_{0}\in(a,b)$ se $\exists r>0:\begin{cases}]x_{0}-r, x_{0}[ \ f(x)<f(x_{0}) \\  ]x_{0}, x_{0}+r[ \ f(x)>f(x_{0})\end{cases}$
- $x_{0}\in (a,b)$ è un punto di min (o max) per $f$ se $\exists r>0:\forall x\in I(x_{0}, r): f(x)\geq (\leq) f(x_{0})$
## TEOREMA DI FERMAT
sia $f:(a,b)\to R$ , sia $x_{0}\in\ ]a,b[$  punto di max o min relativo e supponiamo che $\exists f'(x_{0})$ allora $f'(x_{0})=0$

> il teorema fornisce una condizione necessaria per l'esistenza di punti di estremo relativo all'interno dell'intervallo.
> la condizione non è sufficiente ma necessaria.
...
$f(x_1)=\min_{[a,b]}f$
$f(x_{2})=\max_{[a,b]}f$
> se uno di essi è interno, ad es $x_{1}\in]a,b[$ si ha $f(x_{1})=0$ per il teorema di fermat.
> se $x_{1}=a$ e $x_{2}=b$ o viceversa $\iff \min_{}f=\max_{}f\implies$ $f$ è costante $\implies f'(x)=0   \ \ \ \forall x\in[a,b]$
...
1. $f'(x_{0})\geq 0$    $\forall x\in(a,b)\to$ $f$ crescente in $(a,b)$
	se $x<y$ per il teorema di lagrange $\exists c\in(a,b)\implies f$ strettamente crescente in $(a,b)$
2. (stretta monotonia) $f'(x)>0 \ \ \forall x\in(a,b)\implies f$ strettamente crescente in $(a,b)$
...
sia dato $x$, ad esempio $x>x_{0}$ e applichiamo il teorema di lagrange nell'intervallo $[x_{0},x]$ .
...
per sapere se lo è occorre trovare $r>0$ : in $I(c,r) \ \ f(x)$ è sempre $(\leq) \vee (\geq) f(c)$
...
infatti, se $f'(x) <0$ per $x < c$, $f'(x)>0$ per $x>c$ allora:
a sinistra di $c$, $f$ è decrescente $\implies f(x)>f(c)$

$\begin{cases}\text{a sinistra di } c, f \text{ è decrescente} \implies f(x)>f(c) \\ \text{a destra di } c, f\text{ è crescente}\implies f(x)>f(c)\end{cases}\implies c$ punto di minimo relativo
