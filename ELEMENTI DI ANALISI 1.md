...
Una successione $\{x_{n}\}$ è detta crescente se per ogni $n < m$, con $n,m\in N$, si ha $x_{n}<x_{m}$ ;
Una successione $\{x_{n}\}$ è detta decrescente se per ogni $n<m$, con $n,m \in N$, si ha $x_{n}>x_{m}$ ;
...
Sia $\{a_n\}$ una successione numerica, si dice che $\{a_{n}\}$ diverge a $-\infty$ e scriviamo che $\exists \lim_{ n \to \infty }{a_{n}}=-\infty$ se $\forall k>0 \ \ \exists \psi\in N$ (dipendente da k): $\forall n>\psi$ risulta $a_{n}<-k$

---
$a_{n}$ diverge a $-\infty$ se preso un qualunque numero $k$ maggiore di $0$, esiste un indice di soglia $\psi$ tale che tutti i punti di $a_n$ con $n>\psi$ risulta che i valori di $a_{n}$ saranno minori di $-k$
![[unnamed.png|517]]
...
supponiamo $l>m$, $\epsilon=\frac{l-m}{2}>0$ 
